...
|---|---|---|
| 左上 デジタル魚拓 | テキスト `デジタル魚拓について知りたい` | ホームページ／LIFF申込フォーム |
...
- LINEログインチャネル: `2011607510`（MIHANADAプロバイダー内・公開済み）
- LIFF ID: `2011607510-4wOg38uG`
- LIFF URL: `https://liff.line.me/2011607510-4wOg38uG`
- エンドポイント: `https://www.mihanada.site/digital-gyotaku/order`
- `openid` と `profile` を使用し、リンク済み公式アカウントは `@963wbwmj`。
- ブラウザから受け取ったIDトークンはWorkerからLINEの検証APIへ送り、検証済みの `sub` を注文の `lineUserId` として保存する。画面から渡されたuserIdは信用しない。
- 注文本文と写真はCloudflare KV `MIHANADA_GYOTAKU_ORDERS` に注文番号単位で保存する。受付後は同じLINEユーザーへ注文番号をpushする。
